...
# Copyright (C) 2016-2019 Embecosm Limited
# Contributor Graham Markall <graham.markall@embecosm.com>
# Contributor Simon Cook <simon.cook@embecosm.com>
# Contributor Edward Jones  <ed.jones@embecosm.com>

# This file is a script to run the GCC testsuite on RISC-V using a
# configurable target board and tool chain.
...
parser.add_argument('--triple', dest='triple',
                    default='riscv64-unknown-elf')
parser.add_argument('--arch', dest='arch',
                    default='rv64gc')
parser.add_argument('--abi', dest='abi',
                    default='lp64')
parser.add_argument('--gcc-source', dest='gcc_source',
                    default=os.path.join(TOPDIR, 'gcc'))
parser.add_argument('--timeout', dest='timeout', type=int,
                    default=300)
parser.add_argument('--tools-dir', dest='tools_dir',
...
TEST_SUITE = os.path.join(script_args.gcc_source, 'gcc', 'testsuite')
DG_EXTRACT_RESULTS = os.path.join(script_args.gcc_source, 'contrib', 'dg-extract-results.py')
...
def runtests_env():
    env = os.environ.copy()
    bindir = script_args.tools_dir
    path = '%s:%s' % (bindir, env['PATH'])
    env['PATH'] = path
    env['RISCV_TIMEOUT'] = str(script_args.timeout)
    env['RISCV_TRIPLE'] = script_args.triple
    env['RISCV_SIM_COMMAND'] = script_args.sim_command
    env['RISCV_SIM_FLAGS']   = script_args.sim_flags.strip('"\'')
    env['DEJAGNU'] = os.path.abspath(os.path.join(os.path.dirname(__file__), 'site.exp'))
    return env
